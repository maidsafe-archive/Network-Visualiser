var ConnectionEvents = function(svg) {
    ...
    if (node.group && node.expected) {
      ...
      node.expected.forEach(function(expected) {
        if (actual.indexOf(expected) === -1) {
          svg.select('g#node-' + expected.replace('..', '_') + ' text').classed('blue', false).classed('red', true);
          svg.selectAll('path.link.source-' + node.name.replace('..', '_') + '.target-' + expected.replace('..', '_'))
      ...
      actual.forEach(function(vaultName) {
        if (node.expected.indexOf(vaultName) === -1) {
          svg.select('g#node-' + vaultName.replace('..', '_') + ' text').classed('blue', false).classed('orange', true);
          svg.selectAll('path.link.source-' + node.name.replace('..', '_') + '.target-' + vaultName.replace('..', '_'))
  ...
  var updateConnectionLinks = function(svg, node) {
    ...
    svg.select('svg g#node-' + node.name.replace('..', '_') + ' text').classed(TEXT_NODE_SELECTED_CLASS, true);
    ...
    if (node.group) {
      if (mode === LINK_MODE.CONNECTIVITY) {
        node.group.slice(0, CLOSE_GROUP_LIMIT).forEach(function(vaultName) {
          svg.select('g#node-' + vaultName.replace('..', '_') + ' text').classed('blue', true);
          svg.selectAll('path.link.source-' + node.name.replace('..', '_') + '.target-' + vaultName.replace('..', '_'))
      ...
      if (mode == LINK_MODE.CHURN) {
        ...
        node.group.forEach(function(d) {
          ...
          svg.selectAll('path.link.source-' + node.name.replace('..', '_') + '.target-' + d).classed(className, true);
        });
      }
      svg.selectAll('path.link.target-' + node.name.replace('..', '_')).classed(OVERLAPPING_TARGET_CLASS, true);
    }
  ...
  var revertConnections = function(node) {
    ...
    svg.selectAll('path.link.source-' + node.name.replace('..', '_'))
      .classed('source', false).each(updateNodes('target', false));
    svg.selectAll('path.link.target-' + node.name.replace('..', '_'))
  ...
  var showConnections = function(d) {
    svg.selectAll('path.link.target-' + d.name.replace('..', '_'))
  ...
  var updateNodes = function(name, value) {
    return function(d) {
      ...
      svg.select('#node-' + d[name].name.replace('..', '_')).classed(name, value);
    };
  };
  ...
  this.updateLinksOnLoad = function(nodes) {
    nodes.each(function(node) {
      if (node.group && node.expected) {
        ...
        actual.forEach(function(vaultName) {
          if (node.expected.indexOf(vaultName) === -1) {
            svg.selectAll('path.link.source-' + node.name.replace('..', '_') + '.target-' +
              vaultName.replace('..', '_')).classed('missing', false).classed('not-exp', true);
          }
        });
      }
    });
